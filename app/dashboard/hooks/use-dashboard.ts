...
export function useDashboard() {
  ...
  const fetchEvents = useCallback(
    async (page = 1, status = "", showLoader = true) => {
      try {
        console.log("Fetching events - Page:", page, "Status:", status, "Show loader:", showLoader)
        ...
        if (response.ok) {
          ...
          setError(errorData.error || `Erro ao carregar eventos (${response.status})`)
        ...
        setError("Erro de conexão ao carregar eventos. Verifique sua internet.")
      } finally {
        setIsLoading(false)
  ...
  const handleCopyEvent = useCallback(async (event: Event) => {
    try {
      ...
      return { success: true, message: "Convite copiado para a área de transferência!" }
    } catch (error) {
